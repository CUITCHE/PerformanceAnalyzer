    ...
    if (!pStmt) {
        ...
        do {
            ...
            if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
                ...
                error = [self errorWithMessage:[NSString stringWithUTF8String:sqlite3_errmsg(_db)]];
                if (outErr) {
                    *outErr = error;
                } else {
                    NSLog(@"%@", error);
...
- (BOOL)addFunctionWithName:(NSString*)name maximumArguments:(int)count block:(void (^)(sqlite3_context *context, int argc, sqlite3_value **argv))block {
...
#if ! __has_feature(objc_arc)
    return sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &FMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    return sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &FMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}

int FMDBBlockSQLiteCallBackCollation(void *pArg, int lenOfData1, const void *data1, int lenOfData2, const void *data2);
int FMDBBlockSQLiteCallBackCollation(void *pArg, int lenOfData1, const void *data1, int lenOfData2, const void *data2) {
#if ! __has_feature(objc_arc)
    int (^block)(int lenOfData1, const void *data1, int lenOfData2, const void *data2) = (id)pArg;
#else
    int (^block)(int lenOfData1, const void *data1, int lenOfData2, const void *data2) = (__bridge id)pArg;
#endif
    return block(lenOfData1, data1, lenOfData2, data2);
}

- (BOOL)addCollationWithName:(NSString*)name block:(int (^)(int lenOfData1, const void *data1, int lenOfData2, const void *data2))block {
    if (name == nil) {
        assert(NO);
        return NO;
    }
    ...
    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openCollations to store the values instead. */
    int rt;
#if ! __has_feature(objc_arc)
    rt = sqlite3_create_collation([self sqliteHandle], [name UTF8String], SQLITE_UTF8, (void*)b, &FMDBBlockSQLiteCallBackCollation);
#else
    rt = sqlite3_create_collation([self sqliteHandle], [name UTF8String], SQLITE_UTF8, (__bridge void*)b, &FMDBBlockSQLiteCallBackCollation);
#endif

    if (rt != SQLITE_ERROR) {
        if (!_openCollations) {
            _openCollations = [NSMutableDictionary dictionaryWithCapacity:1];
        }
        [_openCollations setObject:b forKey:name];
        return YES;
    }
    return NO;
}

- (BOOL)removeCollationNamed:(NSString*)name
{
    if (name == nil) {
        return NO;
    }
    ...
    int rt;
#if ! __has_feature(objc_arc)
    rt = sqlite3_create_collation([self sqliteHandle], [name UTF8String], SQLITE_UTF8, NULL, NULL);
#else
    rt = sqlite3_create_collation([self sqliteHandle], [name UTF8String], SQLITE_UTF8, NULL, NULL);
#endif

    if (rt != SQLITE_ERROR) {
        [_openCollations removeObjectForKey:name];
        return YES;
    }
    return NO;
}

- (BOOL)hasCollationNamed:(NSString*)name
{
    return ([_openCollations objectForKey:name] != nil);
}
